...
function TDzHTMLText2.GetBulletStr(const LIParams: TDHTagLIParams): string;
begin
...
function TDzHTMLText2.GetLinkData(LinkID: Integer): TDHLinkData;
begin
  Result := LLinkData[LinkID];
end;

function TDzHTMLText2.GetPngImageFromFile(const PngFileName: string): TPngImage;
begin
  Result := nil;
  if not FileExists(PngFileName) then Exit;
...
function TDzHTMLText2.GetPngImage(const Index: integer): TPngImage;
begin
  Result := nil;
  if not Assigned(FPngCollection) then Exit;
...
function TDzHTMLText2.GetSelectedLinkData: TDHLinkData;
begin
  Result := LLinkData[FSelectedLinkID];
end;

procedure TDzHTMLText2.CMCursorchanged(var Message: TMessage);
begin
...
procedure TDzHTMLText2.SetCursorWithoutChange(C: TCursor);
begin
...
procedure TDzHTMLText2.SetExtraLineSpacing(const Value: TDHLSSmallInt);
begin
...
procedure TDzHTMLText2.SetExtraWordSpacing(const Value: TDHWSSmallInt);
begin
...
procedure TDzHTMLText2.SetInternalMargins(const Value: TDHInternalMargins);
begin
  FInternalMargins := Value;
  FInternalMargins.OnChange := PropsChanged;
end;

procedure TDzHTMLText2.SetTagH1Params(const Value: TDHTagH1Params);
begin
  FTagH1Params := Value;
  Invalidate;
end;

procedure TDzHTMLText2.SetTagH2Params(const Value: TDHTagH2Params);
begin
  FTagH2Params := Value;
  Invalidate;
end;

procedure TDzHTMLText2.SetTagH3Params(const Value: TDHTagH3Params);
begin
  FTagH3Params := Value;
  Invalidate;
end;

procedure TDzHTMLText2.SetTagHRParams(const Value: TDHTagHRParams);
begin
  FTagHRParams := Value;
  Invalidate;
end;

procedure TDzHTMLText2.SetTagLI2Params(const Value: TDHTagLI2Params);
begin
  FTagLI2Params := Value;
  PropsChanged(Self);
end;

procedure TDzHTMLText2.SetTagLIParams(const Value: TDHTagLI1Params);
begin
  FTagLIParams := Value;
  PropsChanged(Self);
end;
...
procedure TDzHTMLText2.SetTagSUPParams(const Value: TDHTagSupParams);
begin
  FTagSUPParams := Value;
  PropsChanged(Self);
end;
...
procedure TDzHTMLText2.CheckMouse(X, Y: Integer);
...
procedure TDzHTMLText2.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
var Handled: Boolean;
begin
...
procedure TDzHTMLText2.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  CheckMouse(X, Y);
...
procedure TDzHTMLText2.CMMouseLeave(var Message: TMessage);
begin
...
procedure TDzHTMLText2.Rebuild;
var B: TBuilder;
begin
...
begin
  ...
  Text := StringReplace(Lb.FText, sLineBreak, '<BR>', [rfReplaceAll]);
  while Text<>'' do
  begin
    A := Text;
    CharIni := A[1];
...
//                22..23: xs := 9;
            ...
            Lb.LWords.Add({$IFDEF FPC}Types.{$ENDIF}Rect(
                X, Y {$IFDEF LINUX}+ dy{$ENDIF},
                X + Ex.Width + dx, Y + Ex.Height {$IFNDEF LINUX}+ dy{$ENDIF}
              ),
...
{$REGION ' ------------ TDHStyleLinkProp ------------ '}
constructor TDHStyleLinkProp.Create(xLb: TDzHTMLText2; xKind: TDHKindStyleLinkProp);
begin
  inherited Create;
...
procedure TDzHTMLText2.SetStyleLink(const Index: Integer;
...
function TDzHTMLText2.GetStoredStyleLink(const Index: Integer): Boolean;
begin
  Result := False;
  case Index of
